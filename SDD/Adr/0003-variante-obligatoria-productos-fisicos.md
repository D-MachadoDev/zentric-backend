...
supersedes: []
related_question: Q-10 (SDD/00-bootstrap/questions-for-owner.md)
```
...
decisión quedaba un estado inalcanzable (riesgo R-10): un producto `Physical` sin
...
El catálogo vigente (`ZENTRIC.md` Dominio 5, CAT-02, `03-value-objects.md`) trata
...
### Sub-decisiones `[PROPUESTO]` (ver Q-12)

Van más allá del texto literal de Q-10 y quedan sujetas a confirmación:
...
- **Positivas:** no existe un producto físico sin unidad de stock; el
  ...
  R-10 queda cerrado. Los digitales conservan su flujo simple.
...
- **Riesgo residual:** si se confirma otra interpretación en Q-12, el ajuste se
  limita a `HasVariant`/`CanBeSold` y a las pruebas asociadas.

## Documentos actualizados en la misma decisión

- `SDD/Domain/01-models.md` — §2 (`Product`: regla de variante obligatoria).
- `SDD/Domain/06-business-rules.md` — nueva **CAT-03**.
- `SDD/00-bootstrap/questions-for-owner.md` — Q-10 resuelta, Q-12 abierta.
- `SDD/00-bootstrap/risks-and-gaps.md` — R-10 cerrada.
- `SDD/00-bootstrap/migration-to-sdd-plan.md` — T-010c ejecutada.
- `SDD/00-bootstrap/spec-conformance-matrix.md`, `current-state.md`,
  `verification-baseline.md` §9, `README.md` y overlay de la skill.
