...
supersedes: []
related_contradiction: C-02 (SDD/00-bootstrap/risks-and-gaps.md)
```
...
- `SDD/Domain/02-aggregates-and-entities.md` §3 y `SDD/Domain/03-value-objects.md`:
  el inventario se lleva por `VariantId` ("Tiene su propio `VariantId` (que sirve
  como SKU para el inventario)"), y §2 define `ProductVariant` como entidad hija
  que "maneja las combinaciones (ej. Talla/Color)".
- `SDD/Domain/01-models.md` §3 y `SDD/ZENTRIC.md` Dominio 6: el inventario está
...
la opción B3, no elegida. Se registra como pregunta abierta **Q-10**.
...
| **B3. `ProductVariant` obligatoria** (toda producto tiene ≥1 variante) | **Aplazada** | Coherente a largo plazo, pero decide sobre la obligatoriedad del catálogo, fuera del alcance de esta decisión. Ver Q-10 |
...
- **Deuda declarada:** `VariantId` y `ProductVariant.Id` son `Guid` planos, no un
  ...
  G-07 / T-004 y se ejecuta de forma consistente para todo el dominio, no solo
  para este caso. Ver `spec-conformance-matrix.md`.
...
- **Riesgo abierto:** un producto físico sin variantes no puede tener inventario.
  Ver Q-10 antes de implementar `InventoryReservationService`.

## Documentos actualizados en la misma decisión

- `SDD/Domain/01-models.md` — §2 (variantes) y §3 (`VariantId`).
- `SDD/Domain/02-aggregates-and-entities.md` — §3 (`VariantId`).
- `SDD/Domain/06-business-rules.md` — nueva INV-03 (unidad de stock).
- `SDD/Domain/services/inventory-reservation-service.md` — entrada por variante.
- `SDD/00-bootstrap/*` — C-02 cerrada, Q-02 resuelta, G-08 cerrada, roadmap,
