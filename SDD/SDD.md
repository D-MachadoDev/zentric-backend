...
> **Rol de este archivo** (skill `generic-sdd-agent` v6.0.0, :0.12 y :13.1-d): este repo **ya declara su SSoT** en [AGENTS.md :0](../AGENTS.md#0-enrutador-y-principios-de-spec-driven-development-sdd) — la carpeta `SDD/`. Por la regla "no se crea una SSoT paralela", este documento **no duplica** las especificaciones: es la **memoria viva** (índice, mapa de entidades, decisiones/ADDENDA, verificación, riesgos y estado) y **apunta** a los documentos canónicos.
>
...
- **Alcance del sistema:** [SDD/01-system-overview.md :3.1](01-system-overview.md) (incluidos) y :3.2 (excluidos: UI, apps móviles, portales, autenticación técnica, tecnologías de implementación y almacenamiento).
...
## 2. Mapa de entidades (procedimiento: [skill :14](../.agents/skills/generic-sdd-agent/SKILL.md#14-mapa-de-entidades-y-auditoria-de-mapeo-completo-anti-amnesia))

`[CONFIRMADO]` por enumeración directa del árbol, de los `.csproj`, de `Program.cs`, de `ZentricDbContext` y de las suites de `Zentric.Tests`.

| ID | Entidad | Tipo | Ubicación | Responsabilidad | Relaciones | Estado | Pruebas |
|---|---|---|---|---|---|---|---|
| E-001 | `User` | Agregado | `Zentric.Domain/Users/User.cs` | Usuario del marketplace, rol único, bloqueo | usa `FullName`, `Email`, `UserRole`, `UserStatus` | parcial (sin `IdentityDocument`) | `Tests/Users/UserTests.cs` |
| E-002 | `Buyer` | Agregado | `Zentric.Domain/Buyers/Buyer.cs` | Comprador, dirección principal y adicionales | usa `PaymentTokens` ([H-03](00-bootstrap/spec-conformance-matrix.md)) | parcial | — `[PENDIENTE]` [T-002b](00-bootstrap/migration-to-sdd-plan.md) |
| E-003 | `Product` | Agregado | `Zentric.Domain/Products/Product.cs` | Catálogo; gestiona variantes y CAT-03 | posee E-004 | implementado (ADR-0002/0003) | `Products/ProductTests.cs` |
| E-004 | `ProductVariant` | Entidad hija | `Products/ProductVariant.cs` | SKU; unidad de stock | usa E-005 | implementado | `Products/ProductVariantTests.cs` |
| E-005 | `VariantAttribute` | Value Object | `Products/ValueObjects/VariantAttribute.cs` | Atributo nombre/valor (Talla, Color) | usada por E-004 | implementado `[PROPUESTO]` ([Q-11](00-bootstrap/questions-for-owner.md#q-11-detalle-del-modelo-de-atributos-de-variante-abierta)) | `Products/VariantAttributeTests.cs` |
| E-006 | `Money` | Value Object | `Products/ValueObjects/Money.cs` | Importe + moneda con aritmética homogénea | usada por E-003, E-009, E-010, E-013 | implementado | `Products/MoneyTests.cs` |
| E-007 | `Inventory` | Agregado | `Inventories/Inventory.cs` | Stock por `(VariantId, WarehouseId)`; contadores Available/Reserved/Damaged/**Used** (`[`:14`](../Zentric.Domain/Inventories/Inventory.cs#L14)`) | clave `VariantId` (ADR-0002) | parcial ([H-01](00-bootstrap/spec-conformance-matrix.md), [H-02](00-bootstrap/spec-conformance-matrix.md), [H-04](00-bootstrap/spec-conformance-matrix.md) abiertos) | `Inventories/InventoryTests.cs` |
| E-008 | `Warehouse` | Agregado | `Warehouses/Warehouse.cs` | Bodega Marketplace o de vendedor | enum `WarehouseType` (`Seller` vs spec `Vendor`) | parcial ([C-05](00-bootstrap/risks-and-gaps.md)) | `Warehouses/WarehouseTests.cs` |
| E-009 | `CustomerOrder` | Agregado | `Orders/CustomerOrder.cs` | Pedido consolidado; estados Cart→Delivered; nace en `Cart` (`[`:49`](../Zentric.Domain/Orders/CustomerOrder.cs#L49)`) | posee E-010; usa E-006 | implementado con desviaciones (`[`:24`](../Zentric.Domain/Orders/CustomerOrder.cs#L24)`, moneda "USD" inventada) | `Orders/CustomerOrderTests.cs` |
| E-010 | `OrderItem` | Entidad hija | `Orders/Entities/OrderItem.cs` | Línea del pedido (variante, cantidad, precio) | usa E-006 | implementado | `Orders/CustomerOrderTests.cs` |
| E-011 | `FulfillmentOrder` | Agregado | `Logistics/FulfillmentOrder.cs` | Despacho por vendedor; nace en `Packed` (`[`:33`](../Zentric.Domain/Logistics/FulfillmentOrder.cs#L33)`) | posee E-012 | **incompleto** (falta `PendingPack`, [C-08](00-bootstrap/risks-and-gaps.md)) | `Logistics/FulfillmentOrderTests.cs` |
| E-012 | `Shipment` | Entidad hija | `Logistics/Entities/Shipment.cs` | Guía/envío por bodega | usada por E-011 | implementado | `Logistics/FulfillmentOrderTests.cs` |
| E-013 | `Invoice` | Agregado | `Billing/Invoice.cs` | Factura maestra/split; fábricas `CreateMaster`/`CreateVendorDetail` | usa E-006 | **incompleto** (falta "Detalle Zentric", [C-08](00-bootstrap/risks-and-gaps.md)) | `Billing/InvoiceTests.cs` |
| E-014 | `ReturnRequest` | Agregado | `Returns/ReturnRequest.cs` | Devolución con doble aprobación; prohíbe digitales | usa `ProductType`, `ReturnStatus` | implementado; **no toca inventario** ([C-08](00-bootstrap/risks-and-gaps.md)) | `Returns/ReturnRequestTests.cs` |
...
| E-030 | ADR-0001…0003 | Decisiones | `SDD/Adr/` | Reserva/fraccionamiento, clave de stock, variante obligatoria | rigen E-007, E-004, E-003 | aprobadas por el Owner | — |
...
| SPEC-001 | Ley funcional Zentric | [`SDD/Domain/ZENTRIC.md`](Domain/ZENTRIC.md) | **congelada (Biblia)** | intacta; ADDENDA en :4 |
...
| SPEC-006 | Trazabilidad de la tanda no registrada ([T-011](00-bootstrap/migration-to-sdd-plan.md)…[T-022](00-bootstrap/migration-to-sdd-plan.md)) | **este documento**, :5 y :7 | **hecha** | riesgo 1; solo documentación, cero código |
| SPEC-007 | Validación de entrada (FluentValidation), RFC 7807 e higiene: cierra [H-09](00-bootstrap/spec-conformance-matrix.md)/[H-11](00-bootstrap/spec-conformance-matrix.md)/[H-12](00-bootstrap/spec-conformance-matrix.md) | **este documento**, :3 · [verification-baseline.md :11](00-bootstrap/verification-baseline.md#11-sexta-iteracion-spec-007-validacion-de-entrada-rfc-7807-e-higiene-2026-09-18) | **hecha** | riesgo 2 · 206/206 pruebas |
...
- INV-01: la Ley permanece intacta. · ERR-01: lo no verificable se marca `[PENDIENTE]`, no se rellena.
...
- [T-02] Registrar contradicciones nuevas ([C-08](00-bootstrap/risks-and-gaps.md), [C-09](00-bootstrap/risks-and-gaps.md), [H-09](00-bootstrap/spec-conformance-matrix.md)…[H-14](00-bootstrap/spec-conformance-matrix.md)) — cubre FR-03 — verificación: [risks-and-gaps.md :2](00-bootstrap/risks-and-gaps.md#2-riesgos-tecnicos) y :6 — estado: **hecha**.
...
| FR-03 | [T-02](00-bootstrap/migration-to-sdd-plan.md) | [risks-and-gaps.md :2](00-bootstrap/risks-and-gaps.md#2-riesgos-tecnicos) ([R-11](00-bootstrap/risks-and-gaps.md)…[R-16](00-bootstrap/risks-and-gaps.md)) y :6 | PASS |
| INV-01 | [T-01](00-bootstrap/migration-to-sdd-plan.md) | `git diff --numstat SDD/Domain/ZENTRIC.md` = 38/1, sin borrado de texto original | PASS |
| CA-01 | [T-01](00-bootstrap/migration-to-sdd-plan.md) | `git status --short` (solo `.md` y `sync-skill.ps1`) | PASS |
| CA-02 | [T-01](00-bootstrap/migration-to-sdd-plan.md) | Este archivo, :1–:8 | PASS |

### SPEC-007 — Validación de entrada, RFC 7807 e higiene · riesgo: 2 · estado: hecha

**Propósito y señal de resultado:** cumplir [AGENTS.md :3.2](../AGENTS.md) (prohibido el `try-catch` genérico en Application), :3.4 (middleware global con RFC 7807), :4.3 (validación de entrada obligatoria con FluentValidation) y el DoD :8 (higiene). Señal: suite en verde con pruebas que demuestren que una entrada inválida se rechaza **antes** del dominio y que una excepción catastrófica ya no se silencia.
...
- INV-01: no se introducen reglas de negocio no especificadas: las reglas de los validadores **espejan** guardas ya existentes (`CustomerOrder`, `OrderItem`, `Money`, `FulfillmentOrder`). · INV-02: no se modifica ningún comportamiento del dominio.
...
| Auditoría anti-amnesia (9 `DbSet` vs agregados de la spec) | **PASS parcial** — los 9 agregados de `SDD/Infrastructure` :2 están mapeados | `Select-String DbSet` |
...
|---|---|---|---|
| ~~**[H-09](00-bootstrap/spec-conformance-matrix.md)**~~ | ~~`AddOrderItemCommandHandler` captura `catch (Exception ex)` y devuelve `Result.Failure(ex.Message)` → **prohibido por [AGENTS.md :3.2](../AGENTS.md)**~~ | **CERRADO en SPEC-007 (2026-09-18):** eliminado el `catch`; la precondición de negocio se informa con `Result.Failure` y las excepciones catastróficas ya no se silencian. Regresión cubierta por `Send_AddOrderItemCommandWhenOrderIsNotInCart_ReturnsBusinessFailureWithoutThrowing` | alta → **cerrada** |
| **[H-10](00-bootstrap/spec-conformance-matrix.md)** | `SDD/Application` [`:3`](../Zentric.Application/Orders/Commands/AddOrderItemCommand.cs#L3) afirma que `AddOrderItemCommand` "valida stock físico usando `IInventoryRepository`": **el puerto no existe y no se valida stock**; `PED-01` (reserva preventiva + timeout 15 min) no está implementada | [SDD/Application/01-use-cases-and-ports.md :15](Application/01-use-cases-and-ports.md)` vs `AddOrderItemCommand.cs` | alta |
...
| **[H-13](00-bootstrap/spec-conformance-matrix.md)** | `ZentricDbContext` mapea **directamente** los agregados de dominio; [AGENTS.md :4.2](../AGENTS.md#42-infrastructure-adapter-agent) exige "aislamiento estricto de las entidades EF Core respecto a Application/Domain". Desviación sin ADR | `ZentricDbContext.cs` | media |
...
`[CONFIRMADO]` Los hallazgos previos **[H-01](00-bootstrap/spec-conformance-matrix.md) … [H-07](00-bootstrap/spec-conformance-matrix.md) siguen abiertos**; **[H-09](00-bootstrap/spec-conformance-matrix.md), [H-11](00-bootstrap/spec-conformance-matrix.md) y [H-12](00-bootstrap/spec-conformance-matrix.md) quedaron cerrados en SPEC-007** (2026-09-18). [H-01](00-bootstrap/spec-conformance-matrix.md) reverificado en el árbol actual: `Inventory.cs:121` valida `AvailableQuantity` y `[`:126`](../Zentric.Domain/Inventories/Inventory.cs#L126)` ejecuta `ReservedQuantity -= quantity` → el reservado puede quedar negativo. Siguen abiertos [H-10](00-bootstrap/spec-conformance-matrix.md), [H-13](00-bootstrap/spec-conformance-matrix.md), [H-14](00-bootstrap/spec-conformance-matrix.md) y la tanda [H-01](00-bootstrap/spec-conformance-matrix.md)…[H-07](00-bootstrap/spec-conformance-matrix.md).
...
| [T-017](00-bootstrap/migration-to-sdd-plan.md) | Suspensión en cascada al bloquear vendedor | **NO hecha** | invariante 6 sin implementar |
...
| Sub-decisiones 1-4 de ADR-0003 | `Adr/0003` | `[PROPUESTO]` → **[Q-12](00-bootstrap/questions-for-owner.md#sub-decisiones-propuesto-ver-q-12)** |
...
### 7.5 Registro de acciones ([skill :17.2](SDD.md) :7)](../.agents/skills/generic-sdd-agent/SKILL.md#172-registro-de-acciones-incidentes-enterprise-y-sesiones-largas-en-sddsddmd-7))
...
|---|---|---|
| 2026-09-18 | `git diff --numstat SDD/Domain/ZENTRIC.md` | La Ley solo tiene adiciones (38/1); el texto original no fue reescrito |
