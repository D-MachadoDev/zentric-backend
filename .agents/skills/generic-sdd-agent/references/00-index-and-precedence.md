...
| `02-modes-and-rigor.md` | Modos A–F y niveles de rigor 1–3 |
...
| `10-zentric-overlay.md` | Overlay del repositorio zentric-backend |

## 2. Jerarquía de fuentes de verdad

Aplica esta prioridad salvo que el repositorio defina otra explícita:

1. Decisión explícita vigente del usuario, product owner o responsable autorizado.
2. Constitución y estándares vigentes del proyecto (`AGENTS.md`, `SDD/`, ADRs).
...
|---|---|
| Instrucciones para agentes y comandos del repositorio | `AGENTS.md` |
| Contexto de negocio global | `SDD/01-system-overview.md` |
| Arquitectura y regla de dependencia | `SDD/02-software-architecture.md` |
| Modelo de dominio, invariantes, lenguaje ubicuo | `SDD/Domain/` |
| Casos de uso, puertos, orquestación | `SDD/Application/` |
| Persistencia, ORM, adaptadores | `SDD/Infrastructure/` |
| Endpoints y exposición | `SDD/Presentation/` |
| Decisiones arquitectónicas duraderas | `SDD/Adr/` |
| Línea base y hallazgos de un proyecto sin contexto | `SDD/00-bootstrap/` |
| Requisitos, diseño, tareas y evidencia de una feature | `SDD/<Contexto>/<id>-<nombre>/` |
